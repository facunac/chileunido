...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$msn_img = ( $row['user_msnm'] ) ? '<img src="' . $images['icon_msnm'] . '" alt="' . $lang['MSNM'] . '" title="' . $lang['MSNM'] . '" border="0" />' : '';
		...
		$template->assign_block_vars('memberrow', array(
			...
			'U_VIEWPROFILE' => append_sid("profile.$phpEx?mode=viewprofileXXXX&amp;" . POST_USERS_URL . "=$user_id"))
		);
