...
function jsvalidateForm(form){
	var valid = true;
	var result = 'Por favor, arregle los siguientes errores:';";
	...
	function form($url = null, $name = null, $method = 'post', $class = null){
		$formTag = "<form action='".$this->Html->url($url)."' class='{$class}' method='{$method}' onSubmit='jsvalidateForm(this); return false;'";
		if ($name != null){
			$formTag .=" name='{$name}'>";
		} else {
			$formTag .='>';
